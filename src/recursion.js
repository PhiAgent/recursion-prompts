...
var sum = function (array) {
    return array.length < 2 ? array.length === 0 ? 0 : array[0] : array.pop() + sum(array);
}
...
var palindrome = function (string) {
    ...
    //after that, you can use recursion to complete
    if (string.length > 2) {
        if (string[0] !== string.slice(-1)) {
            return false;
        } else {
            output = palindrome(string.slice(1, -1));
            return output;
        }
    }
    return false;
...
var mergeSort = function (array) {
    ...
    let leftRecurse = mergeSort(left);
    let rightRecurse = mergeSort(right);
    return mergesorted(leftRecurse, rightRecurse)
};
